 ...
 *
 * MCP tool handler for querying the architecture map database.
 * Provides file→architecture relationships, import graphs, and entity queries.
 */
...
/**
 * Options for the architecture map tool.
 */
export interface ArchitectureMapOptions {
  /** Entity name to find related files */
  entity?: string;
  /** Architecture ID to list files */
  architecture?: string;
  /** File path to get import graph */
  file?: string;
  /** Module/directory path to get full context */
  module?: string;
  /** Import graph traversal depth (default: 2) */
  depth?: number;
...
/**
 * Handle the architecture map tool request.
 */
export async function handleArchitectureMap(
  projectRoot: string,
  ...
  try {
          ...
          text: `Error: Project not initialized with ArchCodex.\n\n` +
            ...
            (nearbyProject
              ? `Found nearby project: ${nearbyProject}\n` +
                `Use: archcodex_map with projectRoot="${nearbyProject}"\n\n`
    ...
    if (options.entity) {
      ...
      return {
        content: [{
          type: 'text',
          text: formatEntityResults(options.entity, files, { markdown: true }),
        }],
      };
    }
    ...
    if (options.architecture) {
      ...
      return {
        content: [{
          type: 'text',
          text: formatArchitectureResults(
    ...
    if (options.module) {
      ...
      return {
        content: [{
          type: 'text',
          text: formatModuleContext(moduleContext, { markdown: true, availableModules }),
        }],
      };
    }
    ...
    return {
      content: [{
        type: 'text',
        text: formatOverview(summary, stats, { markdown: true }),
      }],
    };
  } catch (error) {
    ...
    return {
      content: [{
        type: 'text',
        text: `Error querying architecture map: ${errorMessage}\n\n` +
          `Project root: ${projectRoot}\n\n` +
          `Try checking:\n` +
          `  1. Is the project root correct? Use projectRoot parameter if needed.\n` +
          `  2. Does .arch/ directory exist?\n` +
          `  3. Run with refresh=true to rebuild the database.`,
      }],
      isError: true,
    };
  }
